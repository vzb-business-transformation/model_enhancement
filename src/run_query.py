...
def get_connection():
    return teradatasql.connect(host='tddp.tdc.vzwcorp.com', user=edw_user, password=edw_pass, logmech='ldap')
...
@timing_decorator
def create_temp_sql(query, values):
    """
    Run Teradata multiple SQL query statements
    """
    with get_connection() as connect:
        try:
            ...
            print(f"Temp table query executed successfully")
        except Exception as e:
            print(f"Error executing query: {e}")
